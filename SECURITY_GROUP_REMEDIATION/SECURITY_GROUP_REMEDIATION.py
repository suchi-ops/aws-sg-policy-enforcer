...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    compliant = 'EC2 Security Group {} has no unjustified open ports'
    non_compliant = 'EC2 Security Group {} has public ingress ports {} and public egress ports {} without a valid exception'

    ec2_sg = json.loads(event['invokingEvent'])['configurationItem']['configuration']
    sg_raw_id = ec2_sg['groupId']
    sg_id = sg_raw_id.split()
    #raw_tags = ec2_instance['tags']
    #tags = {}
    ...
    for permission in SecurityGroup['IpPermissions']:
        # If there is no port(s) in the SG rule, move on to the next
        if not permission.get('IpProtocol'):
            continue
        if permission.get('FromPort'):
            ...
            for ip in permission['IpRanges']:
                ...
                if ip['CidrIp'] == '0.0.0.0/0':
                    ...
                    if permission['FromPort'] == permission['ToPort']:
                        open_ports.append(permission['FromPort'])
                    else:
                        open_ports.append(permission['FromPort'])
                        open_ports.append(permission['ToPort'])
        else:
            if permission['IpProtocol'] != '-1':
                ...
                for ip in permission['IpRanges']:
                # 0.0.0.0/0 indicates the port is accessible to the Internet
                    if ip['CidrIp'] == '0.0.0.0/0' and permission['IpProtocol'] == '-1':
                        open_ports.append('1')            
                    
                    

    # Egress Rule checking       
    for egress_permission in SecurityGroup['IpPermissionsEgress']:
        # If there is no port(s) in the SG rule, move on to the next
        if not egress_permission.get('IpProtocol'):
            continue
        if egress_permission.get('FromPort'):
            ...
            for ip in egress_permission['IpRanges']:
                ...
                if ip['CidrIp'] == '0.0.0.0/0':
                    ...
                    # but will only take the from and to port from that range. In a production environment,
                    # you'll likely want to pull out each port in that range to ensure it is accounted for.
                    if egress_permission['FromPort'] == egress_permission['ToPort']:
                        egress_open_ports.append(egress_permission['FromPort'])
                    else:
                        egress_open_ports.append(egress_permission['FromPort'])
                        egress_open_ports.append(egress_permission['ToPort'])
            ...
            if egress_permission['IpProtocol'] != '-1':
                ...
                for ip in egress_permission['IpRanges']:
                # 0.0.0.0/0 indicates the port is accessible to the Internet
                    if ip['CidrIp'] == '0.0.0.0/0' and egress_permission['IpProtocol'] == '-1':
                        egress_open_ports.append('1')
                        print (egress_open_ports)
    ...
    if unjustified_open_ports or unjustified_egress_open_ports:
        return build_evaluation_from_config_item(
            configuration_item,
            'NON_COMPLIANT',
            annotation=build_annotation(non_compliant.format(sg_raw_id, unjustified_open_ports, unjustified_egress_open_ports))
        )
    else:
        return build_evaluation_from_config_item(
            configuration_item,
            'COMPLIANT',
            annotation=build_annotation(compliant.format(sg_raw_id))
        )
